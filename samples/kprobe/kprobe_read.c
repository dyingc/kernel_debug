 ...
 * According to the result of ./kprobe_bind.c, I'm not sure if we should use 
 * "__x64_sys_read" or simply "__sys_read" as the `symbol_name`
 **/

static struct kprobe kp;

static int handler_pre(struct kprobe *p, struct pt_regs *regs) {
    printk(KERN_INFO "Pre-handler: Probed function `read` called\n");
    // Access registers using regs pointer if needed
    return 0;
}

static void handler_post(struct kprobe *p, struct pt_regs *regs, unsigned long flags) {
    printk(KERN_INFO "Post-handler: Probed function `read` completed\n");
    // Access registers using regs pointer if needed
}
